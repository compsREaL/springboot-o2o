$(function () {
    ...
    //页码
    var pageNum = 1;
    //从地址栏URL里尝试获取parent shop categoryId
    ...
    function addItems(pageSize,pageIndex) {
        //拼接出查询的URL，赋空值默认去掉该条件的限制，有值就代表按照该条件去查询
        ...
        //设定加载符，若还在后台读取数据则不能再次访问后台，避免多次重复加载
        loading = true;
        $.getJSON(url,function (data) {
            if (data.success){
                //获取当前查询条件下店铺的总数
                ...
                data.shopList.map(function (item, index) {
                    html+= "<div class='card' data-shop-id='"+ item.shopId +"'>"+
                        ...
                        "<img src='"+ getContextPath() + item.shopImg+"' width='44'>"+"</div>"+
                        "<div class='item-inner'>"+
                        "<div class='subtitle'>"+ item.shopDesc +"</div>"+"</div> "+"</li>"+"</ul>"+"</div>"+"</div>"+
    ...
    $("#shoplist-search-div").on("click",".button",function (e) {
        ...
        if (parentId){
            ...
            //若之前已经选定了其他的category，则移出其选定效果，改成选定新的
    ...
    //需要查询的店铺名字发生变化后，重置页码，清空原先的店铺列表，并加载新的列表
    ...
    //区域信息发生变化后，重置页码，清空原先的店铺列表，并加载新的列表
